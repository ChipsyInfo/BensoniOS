...
open class ENSideMenu : NSObject, UIGestureRecognizerDelegate {
    ...
    open var isMenuOpen : Bool = false
    ...
    open var leftSwipeGestureRecognizer:UISwipeGestureRecognizer!
    /**
    ...
    public init(sourceView: UIView, menuPosition: ENSideMenuPosition, blurStyle: UIBlurEffectStyle = .light) {
        ...
        leftSwipeGestureRecognizer = UISwipeGestureRecognizer(target: self, action: #selector(ENSideMenu.handleGesture(_:)))
    ...
    fileprivate func setupMenuView() {
        ...
        let middleColor = UIColor(hex: 0xC02221)
        let leftColor = UIColor(hex: 0x9F1D1D)
        let  gradient = CAGradientLayer()
        gradient.frame = sourceView.frame
        gradient.startPoint = CGPoint(x: 0.0, y: 0.5)
        gradient.endPoint = CGPoint(x: 1.0, y: 0.5)
        gradient.colors = [(leftColor?.cgColor)!,(middleColor?.cgColor)!,(rightColor?.cgColor)!]
        gradient.locations = [0, 0.5, 1]
        sourceView.layer.insertSublayer(gradient, at: 0)
        sideMenuContainerView.backgroundColor = UIColor(red: 38/255, green: 35/255, blue: 33/255, alpha: 1.0)
    ...
    open func gestureRecognizerShouldBegin(_ gestureRecognizer: UIGestureRecognizer) -> Bool {
        ...
        if gestureRecognizer is UISwipeGestureRecognizer {
            ...
            if menuPosition == .left
            {
                if isMenuOpen
                {
                    if touchPosition.x < menuWidth + 300{
                        return true
                    }
                }
                else
            ...
            else
            {
                if isMenuOpen
                {
                    if touchPosition.x > 0{//CGRectGetWidth(sourceView.frame) - menuWidth {
                        return true
                    }
                }
                else {
