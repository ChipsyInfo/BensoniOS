...
class FacebookViewController: UIViewController,ENSideMenuDelegate,PathMenuDelegate,UITableViewDelegate,UITableViewDataSource {
    var  menuButton:UIButton!
    var  menu:UIBarButtonItem!
    var fbData:FacebookInfoDataController!
    ...
    override func viewDidLoad() {
        ...
        if reachable.isReachable == true {
            ...
            fbData = FacebookInfoDataController()
        ...
        fbTable.delegate = self
        fbTable.dataSource = self
    ...
    {
        DispatchQueue.main.async{
            self.FacebookArray = self.fbData.facebookArray
            if self.fbData.facebookArray.count == 0
            {
            ...
            {
                self.Emptylabel.alpha = 0
                if self.fbData.facebookArray.count == 1
                {
                    let urlString = (self.FacebookArray[0] as AnyObject).value(forKey: "link") as? String
    ...
    func delay(_ delay: Double, closure: @escaping ()->()) {
     ...
     func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell:FacebookItemCell! = tableView.dequeueReusableCell(withIdentifier: "fbCell") as! FacebookItemCell
        cell.cellBlueFbTitle.text = (self.FacebookArray[(indexPath as NSIndexPath).row] as AnyObject).value(forKey: "desc") as? String
        cell.cellWhiteFbTitle.text = (self.FacebookArray[(indexPath as NSIndexPath).row] as AnyObject).value(forKey: "title") as? String
        ...
        let img = (self.FacebookArray[(indexPath as NSIndexPath).row] as AnyObject).value(forKey: "imagepath") as! String
        let appDelegate = UIApplication.shared.delegate as! AppDelegate
        ...
        let task = session.dataTask(with: request, completionHandler: {data, response, error -> Void in
            ...
            if error == nil {
                ...
                DispatchQueue.main.async {
                    ...
                    if let currentcell:FacebookItemCell = cell {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        {
            let urlString = (self.FacebookArray[(indexPath as NSIndexPath).row] as AnyObject).value(forKey: "link") as? String
            let url:URL = URL(string: urlString!)!
            let request = NSMutableURLRequest(url: url)
            let facebkweb = self.storyboard?.instantiateViewController(withIdentifier: "FacebookWebViewController") as! FacebookWebViewController
            facebkweb.request = request
