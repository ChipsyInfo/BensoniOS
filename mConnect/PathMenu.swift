...
open class PathMenu: UIView, PathMenuItemDelegate,CAAnimationDelegate {
    ...
    open var ButtonOpen:Bool! = false
    ...
    override open func point(inside point: CGPoint, with event: UIEvent?) -> Bool {
        if motionState == .expand {
            var checkval:Bool! = false
            for (index, menuItem) in menuItems.enumerated() {
                _ = index
                let val = menuItem.frame.contains(point)
                if val == true
                {
                    checkval = val
                }
    ...
    open func animationDidStop(_ anim: CAAnimation, finished flag: Bool) {
        if let animId = anim.value(forKey: "id") {
            if (animId as AnyObject).isEqual("lastAnimation") {
                self.contentImage = UIImage(named: "fab_more")
    ...
    open func handleTap() {
        ...
        case .close:
            ...
            self.contentImage = UIImage(named: "fab_close")
            setMenu()
